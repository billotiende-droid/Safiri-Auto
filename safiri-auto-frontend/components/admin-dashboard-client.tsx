...
import { vehicleAPI, bookingAPI, type Vehicle, type Booking } from "@/lib/api"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card"
import { Badge } from "@/components/ui/badge"
import { Calendar, Car, CreditCard, LogOut, Plus, Eye } from "lucide-react"
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Welcome, {owner?.name}!</h1>
          <p className="text-muted-foreground mt-1">{owner?.company_name || "Owner Dashboard"}</p>
        </div>
        ...
        {vehicles.length === 0 ? (
          <Card className="border-border text-center py-16">
            <CardContent>
              ...
              <p className="text-muted-foreground mb-4">Add your first vehicle to start renting</p>
              <Link href="/admin/vehicles">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {vehicle.is_verified && <Badge className="bg-green-100 text-green-800">Verified</Badge>}
                      </div>
                      <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                        <span>{vehicle.registration_number}</span>
                        <span>KES {vehicle.price_per_day.toLocaleString()}/day</span>
                      </div>
                    </div>
                    <Link href="/admin/vehicles">
                      <Button variant="outline" size="sm">
        ...
        {bookings.length === 0 ? (
          <Card className="border-border text-center py-16">
            <CardContent>
              ...
              <p className="text-muted-foreground">Your vehicles will appear here when booked</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-4">
